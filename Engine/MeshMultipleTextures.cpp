...
extern int LoadBMPTextureFromFile(const std::string& Filename, GLuint& texture, GLuint programId);
...
std::map <GLuint, size_t> MeshMultipleTextures::PrepareTextures(GLuint programId)
{
	...
	for (auto it = IndexEndPerTexture.begin(); it != IndexEndPerTexture.end(); it++)
	{

		LoadBMPTextureFromFile(it->second, textureHandle, programId); // bind this texture name to TextureHandle
